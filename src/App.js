    ...
    <div className={`min-h-screen ${theme.bg} transition-colors duration-300`}>
      ...
      <section className="relative h-[45vh] min-h-[350px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 text-white">Duvarlarınıza <span style={{color: theme.accent}}>Sanat</span> Katın</h2>
          <p className="text-base text-stone-300 mb-8 max-w-xl">Özgün tasarımlarla mekanlarınıza karakter katın.</p>
          
          {/* GÜNCELLENEN BUTON */}
          <button 
            onClick={() => setShowCollection(true)} 
            className="shimmer-btn text-stone-900 px-10 py-4 rounded-full font-bold text-base shadow-[0_0_20px_rgba(232,220,196,0.3)] hover:shadow-[0_0_30px_rgba(232,220,196,0.5)] hover:scale-105 active:scale-95 transition-all duration-300 flex items-center gap-2 group"
      ...
      {/* Çok Satanlar Kısmı (Dosyanın geri kalanı için referans) */}
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide">
          {products.filter(p => p.isBestSeller).map(product => (
            <div key={product.id} className="flex-shrink-0 w-48 group cursor-pointer" onClick={() => setSelectedProduct(product)}>
      ...
      {/* Admin Panel vb. diğer modallar buraya eklenebilir */}
